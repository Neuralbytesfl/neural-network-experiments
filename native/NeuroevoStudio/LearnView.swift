    ...
    private let topics: [ManualTopic] = [
        ...
        ManualTopic(id: "metrics", title: "Metrics & Overfitting", summary: "Read the chart without fooling yourself.", symbol: "chart.xyaxis.line",
                    sections: [
                        ...
                        ManualSection(title: "Beat a naive baseline", body: "Complexity is useful only when it improves on a simple rule. For classification, this app compares the winner with always choosing the most common training class. For regression, it compares with always predicting the training target mean. A model that does not beat that baseline is evidence that the current data, features, search, or model is not useful yet—not a successful result."),
                        ManualSection(title: "Task-aware metrics", body: "Balanced accuracy averages recall across the classes represented in the test set, so a common class cannot completely hide failures on a rarer one. MSE and mean absolute error use normalized target units in the current evaluator, so preserve the target mean and scale when interpreting them. R-squared is scale-independent and compares squared error with variation in the held-out targets; values below zero mean the model is worse than predicting that held-out mean."),
